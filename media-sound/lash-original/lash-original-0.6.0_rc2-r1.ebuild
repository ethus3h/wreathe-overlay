...
multilib_src_install() {
	...
	if use ladish; then
		rm "${D}"/usr/lib64/liblash.so.1.1.1
		rm "${D}"/usr/lib64/python2.7/site-packages/_lash.so
		rm "${D}"/usr/lib64/python2.7/site-packages/lash.py
		rm "${D}"/usr/lib64/pkgconfig/lash-1.0.pc
		rm "${D}"/usr/lib64/liblash.so
		rm "${D}"/usr/lib64/liblash.so.1
	fi
}
